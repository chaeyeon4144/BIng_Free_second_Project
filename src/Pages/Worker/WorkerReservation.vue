...
<template>
  ...
  <div
    ...
    :key="item.id"
  >
    <p class="profile-h1">배정 대기 목록</p>
    <div class="list-left">
      <ul>
        <li class="progile-h1">{{ item.reservinfo.machine }}</li>
        <li class="progile-h4">일정 | {{ item.reservinfo.date }}</li>
        <li class="progile-h4">주소 | {{ item.reservinfo.address }}</li>
        <li class="progile-h4">
          금액 | {{ Math.floor(item.payment.service * 0.4).toLocaleString() }}원
        </li>
      </ul>
    </div>
    <div class="list-right">
      ...
      <div class="customerinfo">
        ...
        <ul v-show="isCustomerOpen" class="profile-h4">
          <li><span>이름</span>{{ item.customer.name }}</li>
          <li><span>연락처</span>{{ item.customer.mobile }}</li>
          <li><span>이메일</span>{{ item.customer.email }}</li>
          <li><span>주소</span>{{ item.customer.address }}</li>
        </ul>
      </div>
      <div class="reservdetailinfo">
        <button
        ...
        <ul class="profile-h4">
          <li class="firstli">
            <span>서비스 유형</span>{{ item.reservinfo?.type || "-" }}
          </li>
          <li><span>제빙기 정보</span>{{ item.reservinfo?.machine || "-" }}</li>
          <li><span>서비스 주소</span>{{ item.reservinfo?.address || "-" }}</li>
          <li><span>서비스 일시</span>{{ item.reservinfo?.date || "-" }}</li>
          <li><span>추가 서비스</span>{{ item.reservinfo?.plus || "-" }}</li>
          <li><span>요청사항</span>{{ item.reservinfo?.memo || "-" }}</li>
        </ul>
      </div>
    </div>
  </div>
